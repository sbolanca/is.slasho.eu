...
class patTemplate
{
	...
	*/
	...
	// VANNACMS SPECIFIC!!!!!
	function addCMVar( $template, $varname, $variable,$var1=0)
	{
		$template = strtolower( $template );
		$varname  = strtoupper( $varname );
		$allargs=func_get_args();
		if (count($allargs)>4)	$args=','.implode(',',array_slice($allargs,4));
		else $args='';
	...
	*/
	...
	// VANNACMS SPECIFIC
	function addFLDObjects($template, &$object, $ignorePrivate = false) {
		if ($this->exists($template."_fields")) {
			$this->addVar($template."_fields",'cntfld', count($object->_fields)); 
			$this->addObject($template."_fields", $object->_fields, "fld_",$ignorePrivate); 
		} else {
			foreach (get_object_vars($object) as $k=>$v) if ((substr($k,0,8)=="_fields_") &&  ($this->exists($template.$k))) {
				$this->addVar($template.$k,'cnt'.$k, count($v)); 
				$this->addObject($template.$k, $v, "fld_",$ignorePrivate); 
			}			
		}
		if ($this->exists($template."_icons")) {
			$this->addVar($template."_icons",'cntic', count($object->_icons)); 
			$this->addObject($template."_icons", $object->_icons, "icon_",$ignorePrivate); 
		} else {
			foreach (get_object_vars($object) as $k=>$v) if ((substr($k,0,7)=="_icons_") &&  ($this->exists($template.$k))) {
				$this->addVar($template.$k,'cntico'.$k, count($v)); 
				$this->addObject($template.$k, $v, "icon_",$ignorePrivate); 
			}			
		}
	}
