...
include_once("opt/news/news.class.php");
	...
	$crow=new simNews($database);
	$crow->load($id);
	$row->title=$crow->title;
	$tmpl->readTemplatesFromInput( "opt/news/jah/editgallery.html");
	$tmpl->addObject("opt_news_gallery", $crow, "row_",true);
	$tmpl->addVar("opt_news_gallery", "SUBMITACTION", "activateCMCommandPOST('news','addgallery','newsGalleryForm');");
	$tmpl->addVar("opt_news_gallery", "CONTENTID", $id);
	$tmpl->addVar("opt_news_gallery", "ACT", "addgallery");
